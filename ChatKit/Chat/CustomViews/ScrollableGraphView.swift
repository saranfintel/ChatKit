...
@objc open class ScrollableGraphView: UIScrollView, UIScrollViewDelegate, ScrollableGraphViewDrawingDelegate {
    ...
    @IBInspectable open var lineJoin: String = CAShapeLayerLineCap.round.rawValue
    ...
    open var referenceLineLabelFont = UIFont.systemFont(ofSize: 10)
    ...
    private func addReferenceLines(inViewport viewport: CGRect) {
        ...
        referenceLineView?.leftLabelInset = self.leftLabelInset
        referenceLineView?.referenceLineLabelColor = self.referenceLineLabelColor
    ...
    private func dequeue(animation: GraphPointAnimation) {
        ...
        DispatchQueue.global().async {
            if let pointLayer = self.dataPointLayer, let sublayers = pointLayer.sublayers {
                for layer in sublayers {
                    if let ly = layer as? CATextLayer {
                        //print(ly.isHidden)
                        ly.isHidden = false
                        ly.opacity = 1.0
                    }
    ...
    private func updateLabels(deactivatedPoints: [Int], activatedPoints: [Int]) {
        ...
        for point in activatedPoints {
            ...
            let rangeMin = (shouldAutomaticallyDetectRange || shouldAdaptRange) ? self.range.min : self.rangeMin
            var position = calculatePosition(atIndex: point, value: rangeMin)
            //Position change
            position.x = position.x + XPosition // Left space
...
private class BarDrawingLayer: ScrollableGraphViewDrawingLayer {
    ...
    private func createBarPath(centre: CGPoint, color: UIColor) -> UIBezierPath {
        ...
        //        var ori = origin
        //        ori.y = origin.y
        //        let rect1 = CGRect(origin: ori, size: size)
        //        let fillLayer = CAShapeLayer();
        //        fillLayer.path = UIBezierPath(rect: rect1).cgPath;
    ...
    private func createPath () -> UIBezierPath {
        ...
        //changes
        let color: Array<UIColor> = [UIColor.init(hex: "FF3B30"), UIColor.init(hex: "FF9500"), UIColor.init(hex: "FFCC00"), UIColor.init(hex: "5AC8FA"), UIColor.init(hex: "4CD964")]
        for i in activePointsInterval {
            ...
            if let pointLocation = self.graphViewDrawingDelegate?.graphPoint(forIndex: i).location {
                ...
                //Position change
                location.x = location.x + XPosition // Left space
            }
            let colorIndex = i%5
            let pointPath = createBarPath(centre: location, color: color[colorIndex])
            
            let fillLayer = CAShapeLayer();
            let aPath = UIBezierPath.init(rect: pointPath.bounds)
            fillLayer.path = aPath.cgPath
            fillLayer.fillColor = color[colorIndex].cgColor;
            fillLayer.lineJoin = lineJoin
            fillLayer.lineCap = lineCap
            self.addSublayer(fillLayer)
...
private class GradientDrawingLayer : ScrollableGraphViewDrawingLayer {
    ...
    init(frame: CGRect, startColor: UIColor, endColor: UIColor, gradientType: ScrollableGraphViewGradientType, lineJoin: String = "") {
...
private class ReferenceLineDrawingView : UIView {
    ...
    var referenceLineLabelColor: UIColor? = nil
    ...
    var leftLabelInset: CGFloat = 10
    var rightLabelInset: CGFloat = 10
    ...
    private func addLine(withTag tag: String, from: CGPoint, to: CGPoint, in path: UIBezierPath) {
        ...
        let TEXTLABEL_FONT = CGFloat(10.0)
        leftLabel.font = (labelFont == nil) ? UIFont(name:"HelveticaNeue-Light", size:TEXTLABEL_FONT) ?? UIFont.systemFont(ofSize: TEXTLABEL_FONT) : labelFont
        rightLabel.font = (labelFont == nil) ? UIFont(name:"HelveticaNeue-Light", size:TEXTLABEL_FONT) ?? UIFont.systemFont(ofSize: TEXTLABEL_FONT) : labelFont
        
        leftLabel.textColor = (referenceLineLabelColor == nil) ? ChatColor.barChatDarkTheme() : referenceLineLabelColor
        rightLabel.textColor = (referenceLineLabelColor == nil) ? ChatColor.barChatDarkTheme() : referenceLineLabelColor
    ...
    private func boundingSize(forText text: String) -> CGSize {
        let TEXTLABEL_FONT = CGFloat(10.0)
        let font = (labelFont == nil) ? UIFont(name:"HelveticaNeue-Light", size:TEXTLABEL_FONT) ?? UIFont.systemFont(ofSize: TEXTLABEL_FONT) : labelFont

...
+ (VBPiePieceData*) pieceDataWith:(NSDictionary*)object {
    ...
    if ([object isKindOfClass:[NSDictionary class]]) {
        ...
        data.showAmount = [options[@"showAmount"] boolValue];
...
- (void) setPath:(CGPathRef)path {
    ...
    if (_labelsPosition != VBLabelsPositionNone) {
       ...
       // [self.label setFrame:CGRectMake(0, 0, 230, labelSize.height)];
...
- (BOOL) animateToAccent:(double)accentPrecent {
    ...
    CGAffineTransform matrixLabel = CGAffineTransformIdentity;
    matrixLabel = CGAffineTransformMakeTranslation(center.x, center.y);
    matrixLabel = CGAffineTransformTranslate(matrixLabel, (_accentVector.x*_accentValue), (_accentVector.y*_accentValue));
    matrixLabel = CGAffineTransformTranslate(matrixLabel,-center.x,-center.x);

    [CATransaction begin];
    [CATransaction setValue:[NSNumber numberWithFloat:0.35f] forKey:kCATransactionAnimationDuration];
    self.label.affineTransform = CGAffineTransformTranslate(matrixLabel, 0, 0);
    [CATransaction commit];
...
- (void)setFrame:(CGRect)frame {
    ...
    //Frame
    [self.label setFrame:CGRectMake(0, 0, 230, 21)];
}

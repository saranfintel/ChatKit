...
- (void) _updateCharts {
    ...
    for (NSDictionary *options in _chartValues) {
        ...
        [piece setFrame:rect];
        [piece setLabelsPosition:VBLabelsPositionCustom];
            ...
            for (NSInteger i = 0, len = _piecesArray.count; i < len; i++) {
                ...
                if (i+1 < len) {
                    ...
                    [piece setEndAnimationBlock:^{
                        [blockPiece _animate];
                        count = count + 1;
                        if (count == (_piecesArray.count - 1)) {// && self.frame.size.width > 280.0
                            //Remove animation
                            //[self performSelector:@selector(animateOuterManuelly) withObject:nil afterDelay:0.3];
                        }
                    }];
                }
            }
...
- (void) setChartValues:(NSArray *)chartValues animation:(BOOL)animation options:(VBPieChartAnimationOptions)options {
    [self setChartValues:chartValues animation:animation duration:1.2 options:options];
}
...
- (void)animateOuterManuelly {
    for (CALayer *layer in self.layer.sublayers) {
        if ([layer isKindOfClass:[VBPiePiece class]]) {
            VBPiePiece *hitLayer = (VBPiePiece*)layer;
            [hitLayer animateToAccent:_maxAccentPrecent];
        }
    }
}
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    if (self.frame.size.width < 280.0) {
        return;
    }
